namespace DeliveryServiceAPI.Filters
{
	...
	public class LinkRewritingFilter : IAsyncResultFilter
	{
		...
		private static void RewriteLinksInNestedObjects(IEnumerable<PropertyInfo> objectProperties, object obj,
			LinkRewriter rewriter)
		{
			foreach (var objectProperty in objectProperties)
			{
				...
				if (typeInfo.IsClass) RewriteAllLinks(objectProperty.GetValue(obj), rewriter);
			}
		}
		...
		private static void RewriteLinksInArrays(IEnumerable<PropertyInfo> arrayProperties, object obj, LinkRewriter rewriter)
		{
			foreach (var arrayProperty in arrayProperties)
			{
				...
				foreach (var element in array) RewriteAllLinks(element, rewriter);
			}
		}
	}
}